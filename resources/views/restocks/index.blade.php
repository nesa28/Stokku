    ...
    <main class="relative h-full max-h-screen transition-all duration-200 ease-in-out xl:ml-68 rounded-xl">
        <div class="relative z-10 px-6 py-6 max-w-7xl mx-auto">
            ...
            <div class="bg-white shadow-xl rounded-2xl p-6 mx-auto">
                ...
                <div class="mb-6">
                    <form action="{{ route('restocks.search') }}" method="GET" class="mb-6 flex items-center space-x-3">
                        <input type="text" name="search" value="{{ request('search') }}"
                               placeholder="Cari berdasarkan ID Restock atau Supplier..."
                               class="border border-gray-300 rounded-lg px-4 py-2 text-sm max-w-xs focus:outline-none focus:ring-2 focus:ring-blue-400">

                        {{-- NEW: Sorting Dropdown for Restocks --}}
                        <select name="sort_by"
                ...
                {{-- Restock Table (Rest of your existing table code) --}}
                    ...
                    <table class="min-w-full text-left text-sm border-collapse border border-gray-200">
                        <thead class="bg-gray-100 text-slate-700 font-semibold">
                            <tr>
                                ...
                                <th class="px-4 py-2 border-b">Produk yang Direstock</th>
                        ...
                        <tbody>
                            @forelse ($restocks as $restock)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 border-b">{{ $loop->iteration + ($restocks->currentPage() - 1) * $restocks->perPage() }}</td>
                                    ...
                                    <td class="px-4 py-2 border-b">{{ $restock->details->sum('jumlah') }} unit</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-2 text-center text-gray-500">Tidak ada data restock yang ditemukan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
